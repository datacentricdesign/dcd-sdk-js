...
export class PropertyService {
  /**
   * function read a property value as csv.
  ...
  static async read(
    ...
    if (from && to) {
      const readPropertyAPI =
        ...
        '&to=' +
        to;
      return http.GETRequest(readPropertyAPI, token, format);
    } else {
      return http.GETRequest(
        api_url + '/things/' + thing_id + '/properties/' + property_id,
        token
      );
    }
  }
  ...
  static async create(
    ...
    return http.POSTRequest(
      api_url + '/things/' + thing_id + '/properties',
      token,
      property_json
    );
  }
