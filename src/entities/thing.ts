...
export class Thing {
    ...
    async read(): Promise<void> {
        ...
        if (!result.thing) {
            throw new TypeError('body is undefined or null : no thing found, check if the id and token of your thing are valid')
        } else {
    ...
    async find_or_create_property(property_name: string, property_type: PropertyType): Promise<Property> {
        if (this.find_property_by_name(property_name) == undefined) {
            return await this.create_property(property_name, property_type);
        } else {
