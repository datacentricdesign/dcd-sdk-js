...
router.get('/things/:thingId', async (req, res) => {
...
router.get('/persons/:userId', async (req, res) => {
  ...
  const result = await PersonService.readUserId(
    userId,
...
router.get('/things/:thingId/properties/:propertyId', async (req, res) => {
  const thingId = req.params.thingId;
  ...
  const result = await PropertyService.read(
    thingId,
    propertyId,
    req['user'].accessToken,
    from,
...
router.put('/things/:thingId/properties/:propertyId', async (req, res) => {
  const thingId = req.params.thingId;
  const propertyId = req.params.propertyId;
...
router.delete('/things/:thingId', async (req, res, next) => {
  const thingId = req.params.thingId;
...
router.delete('/things/:thingId/properties/:propertyId', async (req, res) => {
  const thingId = req.params.thingId;
  const propertyId = req.params.propertyId;
...
router.post('/things/:thingId/properties', async (req, res) => {
  const thingId = req.params.thingId;
  const body = req.body;
...
router.get('/stats/propertyTypes', async (req, res) => {
  if (!req.query.types) {
    res.status(400).send({
    ...
    if (from && to) {
      console.log(
        'get',
        'api/stats/propertyTypes?from=' + from + '&to=' + to,
        propertyTypes
      );
    } else {
      console.log('get', 'api/stats/propertyType', propertyTypes);
    }
    const result = await StatService.getPropertyTypesStats(
      propertyTypes,
      req['user'].accessToken,
      from,
      to
...
router.get('/tasks/:taskId/resources', async (req, res) => {
  const taskId = req.params.taskId;
  const actor = req.query.actor;
  console.log('get', 'api/tasks/' + taskId + '/resources' + '?actor=' + actor);
  const result = await TaskService.readResources(
    taskId,
    actor,
    req['user'].accessToken
  );
...
router.post(
  '/tasks/:taskId/resources/:resourceId/milestones',
  async (req, res) => {
    const body = req.body;
    const taskId = req.params.taskId;
    ...
    console.log(
      'post',
      'api/tasks/' + taskId + '/resources/' + resourceId + '/milestones',
      body
    );
    const result = await TaskService.addMilestone(
      taskId,
      resourceId,
      body,
      req['user'].accessToken
